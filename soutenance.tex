...
\item<3-> \textbf{Experiment}
	\begin{itemize}
		...
		\item<3-> Similarly, we follow the proper name \textit{Green} from \circled{$M_3$} (incorrectly classified as ORG) to \circled{$M_4$} (correctly classified as PER)
...
\large \textbf{We focus on the word \textit{Kory}, and its performance from model \circled{$M_1$} to \circled{$M_2$}}
	\begin{overprint}
		\onslide<1>
			\begin{center}
			\includegraphics[width=0.4\linewidth]{image2/Chapitre4/M1_383.png}
			\end{center}
		\onslide<2>
			\begin{center}
			\includegraphics[width=0.8\linewidth]{image2/Chapitre4/M2_383.png}
			\end{center}
