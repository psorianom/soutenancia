...
\begin{frame}{Main Challenges and Contributions}
	\begin{enumerate}
		\item<1-> What type of model can we employ to represent a corpus using heterogeneous features?
		...
		\begin{itemize}
		\item<1-> \textit{Hypergraph linguistic model to hold different types of  linguistic information}
		\end{itemize}
		
		\item<2-> How can we combine these features while dealing with feature sparsity?
		\begin{itemize}
		\item<2-> \textit{Multimedia fusion techniques to combine and densify representation spaces}	    	 
		\end{itemize}
		\item<3-> How can we find and employ communities existing within the language networks?
		\begin{itemize}
		\item<3-> \textit{An alternative network-based algorithm to discover semantically related words within a text}
		\end{itemize}
...
We extract useful information from a text based on the distributional hypothesis (a word is defined by its surroundings) 
\begin{itemize}
...
\item<2-> \large\textbf{Main fusion operators:}
	...
	\item<2-> Cross Fusion $X_\gamma(\cdot), X_F(\cdot)$
...
\begin{itemize}
\item<1-> Application of one function to the result of another to produce a new function
\end{itemize}

\item<2-> \textbf{We distinguish three levels of fusion operators}

	\begin{itemize}
	\item<3-> \textbf{First Degree (3 operators):} 
		\begin{itemize}
		\item<3-> $E(\mlex,\msyn)$, $L(\ssyn,\mlex)$ 
		\item<3-> Cross Feature Fusion: $X_F(S^S, M^L)$
		\item<3-> Cross Similarity Fusion: $X_S(S^S, S^L)$
		\end{itemize}
	\item<4-> \textbf{Second Degree (4 operators, 2 shown):} 
		\begin{itemize}
		\item<4-> Cross Feature Early Fusion: $X_F(S^T , E(M^L , M^S ))$
		\item<4-> Late Cross Feature Fusion: $L(M^T, X_F (S^T , M^T ))$
		
		
		\end{itemize}
	\item<5-> \textbf{Higher Degree (2 operators, 1 shown)}
		\begin{itemize}
		\item<5-> Triple Early Double Late Cross Feature Fusion: $E(M_L , E(E(M_T , L(M^T , X_F (S^T , M^T ))) , L(M^L , X_F (S^S , M^L))))$
		\end{itemize}
	\end{itemize}
...
\begin{frame}{Experimental Protocol}
	\begin{itemize}
		...
		\item<2-> \textbf{Test Corpora}
			\begin{itemize}
			\item<2-> CoNLL-2003 (CONLL) \cite{SangM03}: Train: 219,554 lines. Test: 50,350 
			\item<2-> Wikiner (WNER) \cite{Nothman2009}: 3.5 million words. 
			\item<2-> Wikigold (WGLD) \cite{Balasuriya2009}: 41,011 words. 
			\end{itemize}
		\item<3-> \textbf{Learning Algorithm}
			\begin{itemize}
				\item<3-> Structured Perceptron \cite{Collins2002}
			\end{itemize}

		\item<4-> \textbf{Evaluation Metric}
			\begin{itemize}
				\item<4-> F-measure
				\item<4-> Evaluated in a 5-fold CV (WNER and WGLD)
			\end{itemize}
	\end{itemize}	 
	\vspace{\textheight}
\end{frame}
...
\end{itemize}
\item<2-> \large \textbf{Our goal}
\begin{itemize}
\item<2-> Again, to assess the effectiveness of the fusion enriched spaces and to evaluate the pertinence of our community discovering algorithm
\end{itemize}
...
\item<1-> \textbf{Hypergraph Linguistic Model}
	\begin{itemize}
	\item<1-> Considering heterogeneous features to link words together at once using a hypergraph structure
		\begin{itemize}	
		\item<1-> Yields a multi-layered representation of text
...
\item<2-> \textbf{Combining Features and Dealing with Sparsity}
	\begin{itemize}
	\item<2-> Using fusion operators
		\begin{itemize}	
			\item<2-> Intuitive way to leverage the different points of view of each heterogeneous feature while increasing the density of the representation
		\end{itemize}

	\end{itemize}
\item<3-> \textbf{Applications to NLP}
	\begin{itemize}
	\item<3-> Solving NER and WSI/WSD with fusion enriched representations and our community-driven algorithm
		\begin{itemize}	
%		\item Enriched features are indeed useful compared to using single independent features
		\item<3-> A high degree combination of fusion operators are the ones that yield the improvements
		\item<3-> The community finding algorithm improves over similar algorithms while being simpler and allows for heterogeneous features
		\item<3->The Wikipedia-based instantiation serves as a NLP system starting point
...
\item<1-> \textbf{Hypergraph Linguistic Model}
	\begin{itemize}
	\item<1-> Implementing a dataframe-like structure allowing for queries and exploration of large corpora using the proposed model
	\end{itemize}
\item<2-> \textbf{Combining Features and Dealing with Sparsity}
	\begin{itemize}
	...
	\item<2-> Exploring with other modal features
	\end{itemize}
\item<3-> \textbf{Applications to NLP}
	\begin{itemize}
...
\item \footnotesize Sabine Loudcher, Wararat Jakawat, Edmundo-Pavel Soriano-Morales, C\'{e}cile Favre:	\textbf{Combining OLAP and information networks for bibliographic data analysis: a survey}. Scientometrics 103(2)
\end{itemize}
\end{frame}
